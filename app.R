...
server <- function(input, output, session) {
  ...
  observeEvent(input$seed, {
    ...
    if (v$marker_count > 0) {
      for (i in seq_along(v$markers)) {
        leafletProxy("map1") %>% 
          addAwesomeMarkers(data = v$markers[[i]], options = markerOptions(draggable = TRUE), layerId = as.character(i), icon = ico, popup = create_popup_div(i)) %>% 
  ...
  observeEvent(input$friend_marker_click, {
    ...
    v$markers[[input$friend_marker_click]] <- NULL
    v$polys[[input$friend_marker_click]] <- NULL
    ...
    leafletProxy("map1") %>%
      ...
      removeShape(layerId = paste0("poly_", input$friend_marker_click)) %>% 
      addPolygons(data = v$overlappy, color = "red", fillOpacity = 0.5, group = "overlappy")
